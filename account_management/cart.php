...
function show_cart()
{
	...
	if (isset($_SESSION['logged_in_user']))
	{
		...
		{
			$total += $cartitm[3] * $cartitm[5];
			$long_string.=$cartitm[1].",";
			echo '
			<div class="cartitm">
				<div class="cartcol" style="border-right-style: none; margin-left: 3%;">'.$cartitm[1].'</div>
				<div class="cartcol" style="border-right-style: none">'.$cartitm[5].'</div>
				<div class="cartcol style="border-right-style: none">'.$cartitm[3] * $cartitm[5].'</div>
		...
		echo '
			<div class="cartitm" style="margin-top: 3%;">
				...
				<div class="cartcol" style="border-right-style: none">'.$total.' DT </div>
				<form action="account_management/order.php" method="POST" >
				<input type="text" id="login" name="login" style="display: none;" value="'.$_SESSION['logged_in_user'].'">
				<input type="number" id="total" name="total" style="display: none;" value="'.$total.'">
				<input type="text" id="items" name="items" style="display: none;" value="'.$long_string.'">
				<input type="text" id="status" name="status" style="display: none;" value="Pending">
				Address: <input type="text" id="address" name="address" placeholder="Enter your address..." required>
					...
					border-width: thin;
					border-color: black;
					cursor: pointer;" name="buy_cart" >Buy</button>
				</form>
			</div>
			';
	}
